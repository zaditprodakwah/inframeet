...
export default function AuditTrailLogsPage() {
  ...
  const filteredLogs = logs.filter((log) => {
    ...
    const actor = log.profiles?.display_name || log.admin_id || "SYSTEM";
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-400">
            Log akses keamanan read-only sistem mencatat ID Aksi, Aktor Administrator, and Target Entity untuk kepatuhan UU PDP No. 27/2022.
          </p>
        </div>
      ...
      <div className="flex items-center gap-3 p-4 rounded-xl bg-slate-900/40 border border-slate-850">
        ...
        <input
          ...
          placeholder="Cari berdasarkan Tipe Aksi, ID Aktor, Target UUID..."
          ...
          <div>
            {isLoading ? (
              ...
              <div className="text-center py-16 space-y-3 p-6">
                ...
                <p className="text-slate-500 text-[10px]">Log akan terisi otomatis saat terdapat aktivitas pendaftaran, tindakan override admin, or moderasi konten.</p>
              </div>
            ) : (
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="border-b border-slate-850 bg-slate-900/60 text-slate-400 font-bold uppercase tracking-wider text-[10px]">
                    ...
                    <th className="p-4">Aktor Admin</th>
                ...
                <tbody className="divide-y divide-slate-850/80">
                  {paginatedLogs.map((log) => {
                    const actorName = log.profiles?.display_name || log.admin_id || "SYSTEM";
                    const eventAction = log.action_type || "SECURITY_EVENT";
                    ...
                    return (
                      <tr key={log.id} className="hover:bg-slate-900/30 transition-colors">
                        ...
                        <td className="p-4 text-slate-350 truncate max-w-[120px]" title={actorName}>
                          {actorName}
                        </td>
                        <td className="p-4 text-slate-400 font-mono truncate max-w-[100px]" title={log.target_id || "SYSTEM"}>
                          {log.target_id || "SYSTEM"}
                        </td>
                        <td className="p-4 text-center">
                          <button
                            onClick={() => setActiveJson(log)}
          ...
          {activeJson ? (
            ...
            <div className="py-20 text-center space-y-2 text-slate-500">
              ...
              <p className="text-[10px] font-medium leading-relaxed">
                Klik ikon mata (<Eye className="w-3.5 h-3.5 inline" />) pada baris log audit untuk membedah payload forensik secara mendetail.
              </p>
            </div>
          )}
        </div>
