...
import { ShieldCheck, Search, Database, Layers, Eye, RefreshCw } from "lucide-react";
...
export default function AuditTrailLogsPage() {
  ...
  const fetchLogs = async () => {
    setIsLoading(true);
    try {
      // In this sandbox environment, we read from 'audit_log' or fallback to seed logs if empty
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Main Log Grid Table */}
        <div className="lg:col-span-2 overflow-x-auto rounded-2xl border border-slate-850 bg-slate-900/20 backdrop-blur-md">
          {isLoading ? (
                  ...
                  <tr key={log.id} className="hover:bg-slate-900/30 transition-colors">
                    ...
                    <td className="p-4">
                      <span className="px-2.5 py-0.5 rounded bg-indigo-500/10 text-indigo-400 border border-indigo-500/10 font-bold uppercase tracking-wide text-[9px] font-mono">
                        {log.action_type || log.action || "SECURITY_EVENT"}
                      </span>
                    </td>
                    <td className="p-4 text-slate-350 truncate max-w-[120px]" title={log.actor_email || log.actor}>
                      {log.actor_email || log.actor || "SYSTEM"}
                    </td>
                    <td className="p-4 text-slate-400 font-mono">{log.ip_address || "127.0.0.1"}</td>
                    <td className="p-4 text-center">
                      <button
                        onClick={() => setActiveJson(log.details || log.payload || log)}
