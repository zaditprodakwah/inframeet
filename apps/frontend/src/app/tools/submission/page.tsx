...
export default function SubmissionPortalPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#020617] text-slate-900 dark:text-slate-200 pb-20 relative">
      <main className="max-w-7xl mx-auto px-6 py-12 pt-24 lg:pt-32">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <label className={`glass-panel p-6 rounded-3xl cursor-pointer border-2 transition-all group relative bg-white/50 dark:bg-[#0b0f10]/80 shadow-md ${pathway === "b2b" ? "border-indigo-500" : "border-transparent hover:border-indigo-300 dark:hover:border-indigo-500/50"}`}>
                  ...
                  <h3 className="text-xl font-bold mb-2 text-slate-900 dark:text-white">B2B Infrastructure</h3>
                  <p className="text-sm text-slate-600 dark:text-slate-400">Audit sistem aman, verifikasi arsitektur *cloud*, dan strategi mitigasi risiko skala *enterprise*.</p>
                  <div className={`absolute top-4 right-4 transition-opacity ${pathway === "b2b" ? "opacity-100 text-indigo-500" : "opacity-0"}`}>
                    <CheckCircle2 className="w-6 h-6" />
                  </div>
                </label>

                <label className={`glass-panel p-6 rounded-3xl cursor-pointer border-2 transition-all group relative bg-white/50 dark:bg-[#0b0f10]/80 shadow-md ${pathway === "academic" ? "border-emerald-500" : "border-transparent hover:border-emerald-300 dark:hover:border-emerald-500/50"}`}>
                  ...
                  <h3 className="text-xl font-bold mb-2 text-slate-900 dark:text-white">Academic Research</h3>
                  <p className="text-sm text-slate-600 dark:text-slate-400">Pengecekan integritas dataset *peer-reviewed*, verifikasi sitasi kriptografis, dan ringkasan riset.</p>
              ...
              <div className="glass-panel p-8 rounded-3xl space-y-6 bg-white/50 dark:bg-[#0b0f10]/80 shadow-xl border border-slate-200 dark:border-white/5">
                <h4 className="text-xl font-bold text-slate-900 dark:text-white">Brief Parameters</h4>
                <div className="space-y-6">
                  <div>
                    <label className="font-mono text-[10px] font-bold text-slate-500 block mb-2 uppercase tracking-widest">Project Title</label>
                    <input 
                      className="w-full bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 rounded-xl p-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all placeholder:text-slate-400" 
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <select className="w-full bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 rounded-xl p-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500 outline-none appearance-none">
            ...
            <div className="glass-panel p-8 rounded-3xl bg-white/50 dark:bg-[#0b0f10]/80 shadow-xl border border-slate-200 dark:border-white/5">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <h4 className="text-xl font-bold text-slate-900 dark:text-white">Secure Intake Manifest</h4>
              ...
              {files.length > 0 && (
                <div className="mt-6 space-y-3">
                  {files.map((file, idx) => (
                    <div key={idx} className={`flex items-center justify-between p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-white/5 shadow-sm ${file.loading ? 'animate-pulse' : ''}`}>
          ...
          <aside className="lg:col-span-4 space-y-6">
            ...
            {/* ROI Dynamic Summary */}
            <div className="glass-panel p-6 rounded-3xl border-l-4 border-l-indigo-500 bg-white/50 dark:bg-[#0b0f10]/80 shadow-xl">
              ...
              <div className="space-y-5">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-slate-600 dark:text-slate-400">Compute Cycles</span>
                  <span className="font-mono text-sm font-black text-indigo-600 dark:text-indigo-400">12.4 GH/s</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-slate-600 dark:text-slate-400">Storage Allocation</span>
                  <span className="font-mono text-sm font-black text-indigo-600 dark:text-indigo-400">1.2 TB</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-slate-600 dark:text-slate-400">Verification Nodes</span>
                ...
                <div className="pt-5 border-t border-slate-200 dark:border-white/10 mt-2">
                  <div className="flex justify-between items-end">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-2xl font-black text-slate-900 dark:text-white">4,250</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/80 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-900 max-w-md w-full p-8 rounded-3xl text-center border-t-4 border-t-emerald-500 shadow-2xl animate-in slide-in-from-bottom-4">
            <ShieldCheck className="w-16 h-16 text-emerald-500 mx-auto mb-6" />
            <h2 className="text-2xl font-bold mb-3 text-slate-900 dark:text-white">Submission Verified</h2>
