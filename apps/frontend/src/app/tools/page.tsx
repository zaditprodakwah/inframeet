    ...
    <div className="flex flex-col min-h-screen bg-background text-foreground transition-colors duration-300 font-sans transition-colors duration-300">
      ...
      <main className="flex-1 py-12 space-y-16">
        ...
        <section className="max-w-7xl mx-auto px-6 space-y-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-6 rounded-2xl bg-white/80 dark:bg-slate-100 bg-slate-900/60 border border-slate-200/80 dark:border-slate-200 border-slate-800/80 backdrop-blur-md">
            ...
            <div className="relative w-full md:max-w-md">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-slate-950 border border-slate-200 border-slate-800 rounded-xl text-xs text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-all font-medium"
              />
            </div>
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider transition-all border whitespace-nowrap ${
                    selectedCategory === cat
                      ...
                      : "bg-white dark:bg-slate-950 border-slate-200 border-slate-800 text-slate-400 hover:text-white"
                  }`}
                >
        ...
        <section className="max-w-7xl mx-auto px-6 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <Link 
              href="/tools/resume" 
              className="p-6 rounded-3xl bg-gradient-to-br from-indigo-950/20 to-slate-900/40 border border-slate-200/80 dark:border-slate-200 border-slate-800/80 hover:border-indigo-500/40 transition-all duration-300 group hover:-translate-y-1 flex flex-col justify-between h-48"
            >
        ...
        <section className="max-w-7xl mx-auto px-6">
          {loading ? (
            ...
            <div className="text-center py-20 bg-white/70 dark:bg-slate-100 bg-slate-900/40 border border-slate-200/80 dark:border-slate-200 border-slate-800/80 rounded-3xl p-8 max-w-md mx-auto space-y-3">
              <BadgeAlert className="w-8 h-8 text-indigo-500 mx-auto" />
              <p className="text-slate-800 text-slate-200 text-sm font-semibold">Tidak menemukan tools yang cocok.</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredTools.map((tool) => {
                ...
                return (
                  <div 
                    key={tool.id} 
                    className={`relative rounded-3xl border p-6 md:p-8 bg-white/70 dark:bg-slate-100 bg-slate-900/40 backdrop-blur-md flex flex-col justify-between space-y-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-indigo-500/5 ${
                      isSponsor 
                        ? "border-amber-500/30 shadow-md shadow-amber-500/5" 
                        : "border-slate-200/80 dark:border-slate-200 border-slate-800/80"
                    }`}
                  >
                    ...
                    <div className="space-y-4">
                      ...
                      {/* Pricing Tag */}
                      {tool.pricing_info && (
                        <div className="flex items-center gap-1.5 bg-white dark:bg-slate-950/50 p-3 rounded-xl border border-slate-200 border-slate-800/50 text-[10px] text-slate-400">
                          <Layers className="w-3.5 h-3.5 text-indigo-400 shrink-0" />
                          <span className="font-medium">Pricing: </span>
                          <span className="font-bold text-slate-800 text-slate-200">{tool.pricing_info}</span>
                        </div>
                      )}
                    ...
                    <div className="pt-4 border-t border-slate-200/80 dark:border-slate-200 border-slate-800/80 flex items-center justify-between gap-3 text-xs">
                      
                      {/* Embed Badge Option (SEO Mutualism) */}
                      <button
                        onClick={() => setActiveModalTool(tool.name)}
                        className="px-3.5 py-2.5 rounded-xl border border-slate-200 border-slate-800 hover:border-slate-700 text-slate-400 hover:text-white font-bold tracking-wide transition-all shadow-sm shrink-0 flex items-center gap-1.5 text-[11px]"
                      >
