...
export default function MegaMenu() {
  ...
  return (
    <header className="sticky top-0 z-50 backdrop-blur-xl bg-[#020617]/90 border-b border-slate-900 transition-all shadow-sm">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-6">
          ...
          >
            ...
            {isLayananOpen && (
              <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 w-[800px] bg-[#090d1f] border border-slate-800/80 rounded-3xl shadow-2xl p-6 grid grid-cols-3 gap-6 animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    {[
                      ...
                      { label: "Portal Pengajuan", href: "/submission" }
                    ].map((item) => (
                      <Link
                        key={item.label}
                        href={item.href}
                        className="px-2 py-1.5 hover:bg-slate-950 rounded-lg text-xs font-semibold text-slate-400 hover:text-indigo-400 transition-all"
                      >
                        {item.label}
                      </Link>
                    ))}
          ...
          >
            <button className="flex items-center gap-1 text-xs font-bold uppercase tracking-wider text-slate-400 hover:text-indigo-400 transition-colors cursor-pointer py-2 bg-transparent border-0">
              Informasi
              <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-300 ${isInfoOpen ? "rotate-180" : ""}`} />
            </button>

            {isInfoOpen && (
              <div className="absolute top-full left-0 mt-1 w-48 bg-[#090d1f] border border-slate-800/80 rounded-2xl shadow-xl p-3 space-y-1 animate-in fade-in slide-in-from-top-1 duration-150">
                <Link href="/insights" className="block px-4 py-2 text-xs font-bold text-slate-300 hover:text-indigo-400 hover:bg-slate-950 rounded-lg transition-colors">
                  Insights &amp; Edukasi
                </Link>
                <Link href="/case-studies" className="block px-4 py-2 text-xs font-bold text-slate-300 hover:text-indigo-400 hover:bg-slate-950 rounded-lg transition-colors">
                  Studi Kasus Proyek
                </Link>
                <Link href="/about" className="block px-4 py-2 text-xs font-bold text-slate-300 hover:text-indigo-400 hover:bg-slate-950 rounded-lg transition-colors">
                  Tentang Kami
                </Link>
                <Link href="/support" className="block px-4 py-2 text-xs font-bold text-slate-300 hover:text-indigo-400 hover:bg-slate-950 rounded-lg transition-colors">
                  Helpdesk &amp; Panduan
                </Link>
                <Link href="/contact" className="block px-4 py-2 text-xs font-bold text-slate-300 hover:text-indigo-400 hover:bg-slate-950 rounded-lg transition-colors">
                  Hubungi Kami
                </Link>
                <Link href="/login" className="block px-4 py-2 text-xs font-bold text-slate-300 hover:text-indigo-400 hover:bg-slate-950 rounded-lg transition-colors border-t border-slate-800 mt-1 pt-2">
                  Masuk / Akses Dasbor
                </Link>
                <Link href="/join-expert" className="block px-4 py-2 text-xs font-bold text-slate-300 hover:text-indigo-400 hover:bg-slate-950 rounded-lg transition-colors">
                  Daftar Jaringan Pakar
                </Link>
                <Link href="/legal" className="block px-4 py-2 text-xs font-bold text-slate-300 hover:text-indigo-400 hover:bg-slate-950 rounded-lg transition-colors border-t border-slate-800 mt-1 pt-2">
                  Legal &amp; Kepatuhan
                </Link>
              </div>
            )}
        ...
        <button 
          ...
          className="lg:hidden w-10 h-10 rounded-xl border border-slate-800 flex items-center justify-center text-slate-300 hover:bg-slate-900 cursor-pointer"
        >
      ...
      {/* Mobile Drawer */}
      {isMobileMenuOpen && (
        <div className="lg:hidden border-t border-slate-900 bg-[#020617] p-6 space-y-4 animate-in slide-in-from-top-4 duration-200">
          <Link 
            href="/" 
            onClick={() => setIsMobileMenuOpen(false)}
            className="block text-sm font-bold text-slate-300 hover:text-indigo-400 py-2 border-b border-slate-900/50"
          >
            Beranda
          </Link>
          <Link 
