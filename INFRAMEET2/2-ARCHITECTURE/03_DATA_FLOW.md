# INFRAMEET - Data Flow & Operational Sequences

This file consolidates critical sequences in the system, displaying how requests propagate from endpoints, through RLS checks, and trigger calculations.

## Sequence: Request -> RPC -> Database -> Event Log
This sequence visualizes the exact workflow of operations that require high security definer authority (such as proof approvals).

```mermaid
sequenceDiagram
    autonumber
    participant Verifier as Verifier User (JWT)
    participant API as Next.js API Route (/api/proofs/:id/approve)
    participant DB as Supabase PostgreSQL (RPC)
    participant Log as Reputation Logs Table
    participant Directory as Directory Table

    Verifier->>API: POST /api/proofs/123/approve {points: 15} (Auth: Bearer JWT)
    API->>API: Validate Session & Verify Moderator Role
    alt Role Unauthorized
        API-->>Verifier: 403_FORBIDDEN (Mitigation: Reject)
    else Role Authorized
        API->>DB: CALL approve_proof('123', 'verifier_uuid', 15)
        activate DB
        DB->>DB: Verify RLS / Target Proof Exists & is Pending
        DB->>DB: Update trust_proofs status = 'approved'
        DB->>Log: INSERT INTO reputation_logs (points, activity)
        DB->>DB: CALL calculate_trust_score(directory_uuid)
        DB->>Directory: UPDATE trust_score = recalculated_score
        DB-->>API: SUCCESS (proof_id, trust_score)
        deactivate DB
        API-->>Verifier: 200 OK { success: true, trust_score: X }
    end
```
